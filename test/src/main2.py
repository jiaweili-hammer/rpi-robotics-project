...
moveFingers("open",[0.8,0.8,0.8],'j2s7s300',3)
...
for i in range(len(tower_x)):
    tower_x_new.append(tower_x[i]+0.06)
    tower_y_new.append(tower_y[i]+0.06)
"""
...
while i < 14:
    ...
    print("close gripper")
    moveFingers("close",[-0.5,-0.5,-0.5],'j2s7s300',3)
    
    #attach the block to the end-effector
    req = AttachRequest()
    req.model_name_1 = "j2s7s300"
    req.link_name_1 = "j2s7s300_link_7"
    ...
    #MOVE ARM TO DESIRED X-Y POSITION FOR BLOCK TOWER TO WAYPOINT 1
    pose_goal.orientation.w = 0
    pose_goal.orientation.x = 1
    pose_goal.orientation.y = 0
    pose_goal.orientation.z = 0

    pose_goal.position.x = tower_x[i]/2
    pose_goal.position.y = tower_y[i]/2
    pose_goal.position.z = z_height_reset
    ...
    #MOVE ARM TO DESIRED X-Y POSITION FOR BLOCK TOWER FROM WAYPOINT 1
    pose_goal.orientation.w = 0
    pose_goal.orientation.x = 1
    pose_goal.orientation.y = 0
    pose_goal.orientation.z = 0
